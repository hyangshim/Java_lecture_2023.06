...
public class Ex07_Subsring {
	public static void main(String[] args) {
		String ssn = "880823-1234567";
		
		String firstPart=ssn.substring(0,6);   // 0이상 6미만
		System.out.println(firstPart);
		
		String secondPart =ssn.substring(7);  // 7부터 끝자리
		System.out.println(secondPart);
		
		System.out.println("생년월일을 \"1988-08-23\"형태로 바꾸기");
	
		if (ssn.substring(7,8).equals("1")) {
			System.out.println("19"+ssn.substring(0,2)+"-"+ssn.substring(2,4)+"-"+ssn.substring(4,6));
		}else if (ssn.substring(7,8).equals("2")) {
			System.out.println("19"+ssn.substring(0,2)+"-"+ssn.substring(2,4)+"-"+ssn.substring(4,6));
		}else if (ssn.substring(7,8).equals("3")) {
			System.out.println("20"+ssn.substring(0,2)+"-"+ssn.substring(2,4)+"-"+ssn.substring(4,6));
		}else if (ssn.substring(7,8).equals("4")) {
			System.out.println("20"+ssn.substring(0,2)+"-"+ssn.substring(2,4)+"-"+ssn.substring(4,6));
		}
